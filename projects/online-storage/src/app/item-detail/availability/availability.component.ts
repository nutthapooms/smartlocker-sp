...
export class AvailabilityComponent implements OnInit {
  ...
  notifyMe(container: ItemDetailAvailabilityContainer) {
    this.http.post<WatchDTO>(`http://192.168.1.34:8080/api/online/items/${this.item.id}/watch`, {
  ...
  unnotifyMe(container: ItemDetailAvailabilityContainer) {
    this.http.delete<number>(`http://192.168.1.34:8080/api/online/items/${this.item.id}/watch/${container.watch.id}`).subscribe(data => {
