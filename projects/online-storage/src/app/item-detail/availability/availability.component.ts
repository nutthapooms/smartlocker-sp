...
export class AvailabilityComponent implements OnInit {
  ...
  fetchCountires() {
    let endpoint = `http://http://192.168.1.34:8080/api/online/browse-locations`
  ...
  notifyMe(container: ItemDetailAvailabilityContainer) {
    this.http.post<WatchDTO>(`http://http://192.168.1.34:8080/api/online/items/${this.item.id}/watch`, {
  ...
  unnotifyMe(container: ItemDetailAvailabilityContainer) {
    this.http.delete<number>(`http://http://192.168.1.34:8080/api/online/items/${this.item.id}/watch/${container.watch.id}`).subscribe(data => {
