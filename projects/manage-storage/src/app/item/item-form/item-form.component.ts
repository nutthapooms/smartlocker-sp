...
export class ItemFormComponent implements OnInit {
  ...
  onFileSelected(event) {
    if (event.target.files.length > 0) {
      ...
      this.http.post<number>('https://smartlocker20220922110147.azurewebsites.net/api/admin/images/', formData)
        .subscribe(res => {
  ...
  submitForm() {
    let endpoint: string;
    if(this.itemId != null) {
      endpoint = `https://smartlocker20220922110147.azurewebsites.net/api/admin/items/${this.itemId}`
     } else {
      endpoint = `https://smartlocker20220922110147.azurewebsites.net/api/admin/items`
     }
