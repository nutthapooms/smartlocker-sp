...
export class LockerFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.lockerId != null) {
      this.http.get<LockerDTO>(`https://smartlocker20220922110147.azurewebsites.net/api/admin/lockers/${this.lockerId}`).subscribe(data => {
        console.log(data)
        this.locker = data;
      })
    }
  ...
  submitForm() {
    let endpoint: string;
    if(this.lockerId != null) {
      endpoint = `https://smartlocker20220922110147.azurewebsites.net/api/admin/lockers/${this.lockerId}`
     } else {
      endpoint = `https://smartlocker20220922110147.azurewebsites.net/api/admin/lockers`
     }
